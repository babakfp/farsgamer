...
<Form class="grid gap-8 mt-5 p-6 bg-white rounded md:mt-8">
	<div>
		...
		<RadioboxWrapper
			class="flex flex-wrap gap-2 lg:flex lg:justify-start"
			name="account-categories"
			bind:selected={accountCategory}
		>
	...
	<Checkbox name="fast-delivery" bind:checked={fastDelivery}>
		...
		<span class="text-sm mr-1"
			>(<span class="font-bold">50,000</span> <span class="text-xs">تومان</span>)</span
		>
	</Checkbox>
	...
	<div class="grid gap-8 xs:flex xs:flex-row-reverse xs:items-end xs:justify-between">
		<div class="min-w-50">
			<div class="flex items-center justify-between gap-8">
				...
				<span class="-mb-2 pt-2 pb-4 px-6 bg-red-400 rounded-t text-white text-xs font-bold"
					>٪۲۰</span
				>
			</div>
			<div
				class="relative flex items-center justify-end gap-1 py-3 px-8 bg-gray-50 rounded shadow-b-sm text-lg"
			>
		...
		{:else}
			<button
				class="btn btn--submit w-full sm:max-w-60 btn--brand font-medium text-base"
				type="submit"
