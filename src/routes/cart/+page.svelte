			...
			<Form class="grid gap-8 p-4 bg-white rounded">
				...
				<ul>
					...
					{#if $cartDiscountInUse}
						<PriceItem
							title="تخفیف"
							price={numberToPrice($cartDiscountInUse.value)}
							negative={true}
						>
							<button
								class="btn p-0 h-6 w-6 hover:text-red-500"
								slot="after-price"
								on:click={() => cartDiscountInUse.set(null)}
							>
				...
				<Checkbox class="text-sm" name="use-wallet" bind:checked={useWallet}
					>استفاده از کیف پول <span class="text-xs">({$walletBalance} تومان)</span></Checkbox
				>
				<Coupon
					classContainer="-mt-4"
					name="discount-code"
					btnText="اعمال تخفیف"
					bind:value={discountFieldValue}
					on:submit={applyCoupon}
				...
				<div class="-mt-4">
					<button
						class="btn btn--brand btn--submit w-full {!$isLoggedIn && 'cursor-not-allowed'}"
						disabled={!$isLoggedIn}
						type="submit">ادامه تسویه حساب</button
					>
					{#if !$isLoggedIn}
