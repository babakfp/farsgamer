...
<div class="mb-8 bg-white rounded lg:mb-0 lg:min-w-80">
	...
	<nav>
		...
		<button
			class="flex items-center gap-3 w-full py-3 pb-4 pl-6 pr-5 {show &&
				'border-t border-gray-100'} hover:text-brand lg:hidden"
			on:click={() => (show = !show)}
		>
			<i
				class="icon-caret-down w-8 h-8 flex items-center justify-center text-lg opacity-75 duration-300 ease-in-out rotate-0 {show &&
					'!rotate-180'}"
			/>
