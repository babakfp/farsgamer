...
<div class="3xs:relative">
	...
	<OutClick on:outclick={() => (isOpen = false)} excludeByDomNode={[toggle]}>
		<div
			class="z-20 absolute inset-x-4 top-full -mt-2 overflow-hidden bg-white border border-gray-100 rounded-sm duration-150 hide -translate-y-8 {isOpen &&
				'show !translate-y-0'} 3xs:left-0 3xs:right-auto 3xs:w-80 xs:w-88"
		...
		>
			<div class="flex items-center justify-between p-3 border-b border-gray-100 text-sm">
				<span>اعلان‌ها</span>
				<a class="link text-xs | -ml-3 -my-3 pl-3 py-3" href="/dashboard/announcements"
					>مشاهده همه</a
				>
			</div>
			...
			<div class="py-2">
				...
				{:else}
					<p class="h-22 flex items-center justify-center text-sm text-center text-gray-500">
						چیزی پیدا نشد.
					</p>
				{/if}
			</div>
