<script>
	...
	onMount(() => {
		...
		if (TabLabels.length !== TabPanels.length)
			throw new Error("<TabLabel /> and <TabPanel /> length must be the same!")
		...
		for (let i = 0; i < TabItemsLength; i++) {
			if (
				$page.url.hash &&
				TabPanels[i].id &&
				TabPanels[i].id === $page.url.hash.replace("#", "")
			) {
				activateItem(i)
				activeIndex = i
			...
			TabLabels[i].addEventListener("click", () => {
				if (
					TabLabels[i].classList.contains("active") &&
					TabPanels[i].classList.contains("Tab--active")
				)
					return
				activateItem(i)
				activeIndex = i
...
<div class="Tab {className}" bind:this={Tab}>
	<nav>
		<ul
			class="tabs flex items-center pb-2 -mb-2 scrollbar-x scrollbar-x-spacing-lg overflow-y-hidden"
		>
			<slot name="tab" />
		</ul>
	</nav>
...
<style lang="postcss">
	...
	.Tab :global(.Tab__TabLabel) {
		@apply whitespace-nowrap;
	}
	.Tab :global(.Tab__TabPanel) {
		@apply hidden;
	}
	.Tab :global(.Tab__TabPanel.Tab--active) {
		@apply block;
	}
</style>
